...
$tipo_torta =  $data_set->get_items_key("tipo_torta");
$tipo_farcitura_torta =  $data_set->get_items_key("tipo_farcitura_torta");
$modello_torta =  $data_set->get_items_key("modello_torta");
$allestimento_torta =  $data_set->get_items_key("allestimento_torta");
...
<h1>Tipo di Torta</h1>
<?php
	foreach($tipo_torta as $key => $value) { if($key > 1) echo '<input type="radio" id="'.$value.'" name="tipo_torta" value="'.$value.'"><label style="width: 100%;" for="'.$value.'">'.$value.'</label>'; }
?>
<h1>Farcitura</h1>
<?php
	foreach($tipo_farcitura_torta as $key => $value) { if($key > 1) echo '<input type="radio" id="'.$value.'" name="tipo_farcitura_torta" value="'.$value.'"><label style="width: 100%;" for="'.$value.'">'.$value.'</label>'; }
?>
<h1>Modello </h1>
<?php
	foreach($modello_torta as $key => $value) { if($key > 1) echo '<input type="radio" id="'.$value.'" name="modello_torta" value="'.$value.'"><label style="width: 100%;" for="'.$value.'">'.$value.'</label>'; }
?>
<h1>Allestimento</h1>
<?php
	foreach($allestimento_torta as $key => $value) { if($key > 1) echo '<input  onchange="$(\'#note_allestimento_torta'.$key.'\').show();"  type="checkbox" id="'.$value.'" name="allestimento_torta'.$key.'" value="'.$value.'"><label style="width: 100%;" for="'.$value.'">'.$value.'</label><br>
	<input  type="text" id="'.$value.'" name="note_allestimento_torta'.$key.'" id="note_allestimento_torta'.$key.'" placeholder="Descrizione">'; }
?>
