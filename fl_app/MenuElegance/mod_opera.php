...
require_once("../../fl_core/core.php"); 
	...
	if(isset($_GET['id1'])) {
		...
		if(mysql_num_rows ($risultato) > 0){			//se la query trova la portata nel menu in synapsy effettua la cancellazione
			...
			$restore = "SELECT * FROM `fl_synapsy` WHERE type1 = '" . $type1 . "' AND type2 = '" . $type2 . "' AND id1 = '" . $id1 . "' AND id2 = '" . $id2 . "' LIMIT 1";
			$risultato = mysql_query($restore, CONNECT);
			$restore = mysql_fetch_assoc($risultato);
			action_record('RM-PORTATA','fl_synapsy',$id1,base64_encode(json_encode($restore)));
	...
	if(isset($_POST['synapsy_id'])) {
		...
		foreach($_POST as $key => $val) { 
			...
			if($key != 'synapsy_id' && $val != '') {

				if(is_array($val)) $val = implode(' e ', $val); 
				$note .= ucfirst(str_replace('_',' ',$key)).':'.$val.','; 
			} 
		}
		$note = rtrim($note,",");
	...
	if(isset($_GET['disaccoppia'])) {
		$id = check($_GET['disaccoppia']);

		$restore = "SELECT * FROM `fl_synapsy` WHERE id = ".$id ." LIMIT 1";
		$risultato = mysql_query($restore, CONNECT);
		$restore = mysql_fetch_assoc($risultato);
		action_record('DISACCOPPIA','fl_synapsy',$id,base64_encode(json_encode($restore)));
