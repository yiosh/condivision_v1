...
$("h3 > a").click(function(e){
	...
	$.ajax({
		...
		error: function(xhr, status, error){
			alert("Portata non inserita: " + error);			
		}
	});
	return false;
});
...
<p>
Categoria: 
<input type="radio" value="Tutto" id="boxCat" name="categoria_ricetta"><label for="boxCat">Tutto</label>
...
<!--Tipo di Servizio:
<?php if($portata_id < 5) {
foreach($tipo_servizio_evento AS $key => $val) { if($key < 2 ) { $val = 'Tutti';  } ?>

<input type="radio" value="<?php echo $key; ?>" id="b<?php echo $key; ?>" name="tipo_servizio_evento"><label for="b<?php echo $key; ?>"><?php echo $val; ?></label>
...
$query = "SELECT * FROM fl_ricettario WHERE attivo = 1 AND variante < 1 AND portata = $portata_id;"; 
...
echo '<div class="boxCat a'.$riga_res['categoria_ricetta'].' b'.$riga_res['tipo_servizio_evento'].'" >';
echo '<span style="'.$scelto.'" title="'.strip_tags(converti_txt($riga_res['note'])).'"><h2>'.converti_txt($riga_res['nome']).'</h2>'.$aggiungi.'</span>';
//echo '<a title="'.converti_txt($riga_res['nome']).': '.strip_tags(converti_txt($riga_res['note'])).' '.$prezzo.'" rel="gallery" href="'.$img.'" class="fancybox">
echo "<a title=\"".converti_txt($riga_res['nome'])."\" href=\"#\" onclick=\"OpenBrWindow('".ROOT.$cp_admin."fl_app/MenuElegance/viewer.php?portata=".$portata_id."&startSlide=".$counter."','Componi','fullscreen scrollbars'); return false;\">";
echo '<img src="'.$img.'" alt="'.strip_tags(converti_txt($riga_res['note'])).'" title="'.strip_tags(converti_txt($riga_res['note'])).'" /> </a>'; 
echo '</div>';
...
$portata = array('','Aperitivi','Antipasti','Primi','Secondi','Frutta','Dessert','Torte');
