...
<h1>Farcitura</h1>
<?php
	foreach($tipo_farcitura_torta as $key => $value) { if($key > 1) echo '<input type="checkbox" id="'.$value.'" name="tipo_farcitura_torta[]" value="'.$value.'"><label style="width: 100%;" for="'.$value.'">'.$value.'</label>'; }
?>


<?php
	foreach($allestimento_torta as $key => $value) { if($key > 1) echo '<h1>'.$value.'</h1><input   type="text" id="'.$value.'" name="allestimento_torta[]" value="" placeholder="Es. '.$value.'">'; }
?>
