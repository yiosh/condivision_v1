...
$(document).ready(function(){
	...
	ambiente_id = getUrlParameter('ambiente_id');									//ambinete dall' url
	template_id = getUrlParameter('template_id');									//template_id dall' url
	tables = 0;


	$.get('mod_template.php',{ evento_id : evento_id , ambiente_id : ambiente_id , info: 1, template_id : template_id },function (data) {
	...
	$('#U_name').keyup(function (e) {           						//azione nel modale riguardo il nome del tavolo
		...
		$.get('mod_template.php',{ template_id:template_id,tableNameUser : escape(retrievedtext) , tableId : tableId , evento_id : evento_id});	//inserisce il tavolo con il nome scritto
	...
	$('#catTable').change(function (e) {
		...
		$.get('mod_template.php',{ template_id:template_id,categoria : categoria , tableId : tableId , evento_id : evento_id});	//inserisce il tavolo con il nome della categoria
	...
	$('#numTable').keyup(function (e) {
		...
		$.get('mod_template.php',{template_id:template_id, numero : numero , tableId : tableId , evento_id : evento_id});	//inserisce il tavolo con il nome della categoria
		object._objects[2].setText(categoria + ' ' + numero);//setto il testo ricevuto nel tavolo
	});
	...
	$('#createElement').on('click',function() {
		...
		var numero = '';	//numero utente
		var retrievedtext = '';								//nome tavolo utente
	...
	$(document).on("click", '#delTable',function () {
		...
		if (activeObject) {
			if (window.confirm('Sei sicuro di voler eliminare il tavolo?')) {
				$.get('mod_template.php',{deleteTable : 1, delTable : delTable , evento_id : evento_id,template_id:template_id}); //cancella tavolo
	...
	$(document).on('click','#print',function(){
		...
		$.post('mod_template.php',{img : data , evento_id: evento_id,template_id},function (data){
			var parsed = $.parseJSON(data);
			if (parsed.esito == 1) {
				...
				$.fancybox({
					href: 'mod_stampa.php?template_id='+template_id+'&orientamento='+orientamento+'&evento_id='+evento_id,
					autoSize : false,
					width    : "100%",
					height   : "100%",
					title : 'Schema Tavoli',
...
canvas.on('object:modified', function (options) { //modifica coordinate dell'oggeto spostato
	...
	$.get('mod_template.php',{ updateTable : 1 ,TableId : tableId, evento_id : eventoIf , x : x, y:y, angle:angle ,template_id:template_id });
	canvas.deactivateAll();
	canvas.renderAll();
});
...
function createTable (marginTop,marginLeft,name,textInsert,fila,type,categoria,numero){
	...
	$.get('mod_template.php',{ template_id : template_id,insertTableId : name, evento_id : evento_id , text : escape(textInsert) ,
...
function createTableOneP (name,diverso = false){
	...
	$.get('mod_template.php',{ template_id : template_id,insertTableId : name, evento_id : evento_id , diverso : diverso },function (data) {
