...
<?php
	...
	$query = "SELECT persone.*, tavolo.numero_tavolo,tavolo.nome_tavolo,tavolo.id,tavolo.evento_id FROM `fl_tavoli_commensali` AS persone LEFT JOIN fl_tavoli AS tavolo ON persone.tavolo_id = tavolo.id WHERE tavolo.`evento_id` = '$evento_id' ORDER BY persone.`cognome` ASC;";
