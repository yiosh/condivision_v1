...
$(document).ready(function(){
	...
	$.get('mod_opera.php',{ eventId : evento_id },function (data) {
	...
	textColor = '#674242';												//colore del testo	
	...
	var tavoloBase = new fabric.Circle({								//tavolo preso a modello
		...
		stroke: '#674242',												//colore bordi del cerchio
  	...
  	var tavoloBase1 = new fabric.Rect({
		...
		stroke: '#674242',												//colore bordi del rettangolo
  		strokeWidth: 1,													//grandezza dei bordi
  		fill: '#fff'
  	});
  	...
  	var tavoloBase2 = new fabric.Ellipse({
		...
		stroke: '#674242',												//colore bordi dell'ellisse
  		strokeWidth: 1,													//grandezza dei bordi
  		fill: '#fff'
  	});
	...
	canvas.on('mouse:dblclick', function (options) { 					//evento doppio click
		...
		if(object != null){												//se si è cliccato su un oggeto si apre il modale
...
canvas.on('object:modified', function (options) { //modifica coordinate dell'oggeto spostato
	...
	$.get('mod_opera.php',{ updateTable : 1 ,TableId : tableId, eventoId : evento_id , x : x, y:y, angle:angle  });
	canvas.deactivateAll();
	canvas.renderAll();
});
...
function createTable (marginTop,marginLeft,name,textInsert,fila,type,categoria,numero){
	...
	$.get('mod_opera.php',{ insertTableId : name, insertEventId : evento_id , text : escape(textInsert) ,
		x : marginLeft, y:marginTop,type : type , categoria : categoria , numero : numero},function (data) {
...
/*************************************************/
function createTableOneP (name){

	$.get('mod_opera.php',{ insertTableId : name, insertEventId : evento_id },function (data) {
